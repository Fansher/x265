...
{
    //64位长的sum_ssd，低32位存储的当前块所有元素和，高32位存储的当前块所有元素的平方和
...
//返回当前块YUV的AC能量，只在LookaheadTLD::calcAdaptiveQuantFrame函数中被调用
...
void LookaheadTLD::calcAdaptiveQuantFrame(Frame *curFrame, x265_param* param)
{
    ...
    int maxCol = curFrame->m_fencPic->m_picWidth;   //原始宽
    int maxRow = curFrame->m_fencPic->m_picHeight;  //原始高
    int blockCount, loopIncr;
    float modeOneConst, modeTwoConst;
    //根据不同的块大小赋值不同的参数
    //qgSize表示能够调整qp的最小CU大小（可取64，32，16，8）
    ...
    //初始化为0， 用于累加整帧YUV通道像素的平方和，和整帧像素的和
    for (int y = 0; y < 3; y++)
    {
            ...
            if (param->bEnableWeightedPred || param->bEnableWeightedBiPred)
            {
                for (int blockY = 0; blockY < maxRow; blockY += loopIncr)
                    for (int blockX = 0; blockX < maxCol; blockX += loopIncr)
                        acEnergyCu(curFrame, blockX, blockY, param->internalCsp, param->rc.qgSize); //计算每个16x16的AC能量：n*Variance 值（像素没有下采样！！！）
            }
        }
        else
        {   //使用空域AQ算法
            if (param->rc.hevcAq)
            {   //如果开启了新的AQ算法，则走该方法计算qp偏移量
...
void LookaheadTLD::lowresIntraEstimate(Lowres& fenc, uint32_t qgSize)
{
    ...
    pixel* samples = neighbours[0], *filtered = neighbours[1]; //samples是参考周边原始数据，filtered是经过intra滤波后的数据
    ...
    for (int cuY = 0; cuY < heightInCU; cuY++)
    {
        ...
        for (int cuX = 0; cuX < widthInCU; cuX++)
        {
            ...
            pixCur -= fenc.lumaStride + 1; //假设当前块的左上角像素点坐标为(x,y) ，则当前位置为（x-1，y-1)，等于是找到左上角那个参考像素的位置
            ...
            primitives.cu[sizeIdx].intra_filter(samples, filtered); //将相邻行参考像素数据进行滤波（intra预测不是对原始像素哦，原始像素需要滤波才行~）
            ...
            fenc.rowSatds[0][0][cuY] += icostAq; //累加当前行的icostAq，注意数组前2个下标是[0][0]
        }
    }
...
//向输入列表m_inputQueue中添加原始帧准备帧类型决策，在buffer满时，触发帧类型决策
void Lookahead::addPicture(Frame& curFrame, int sliceType)
{
    ...
    {
        checkLookaheadQueue(m_inputCount);        //判断计算m_filled
...
//判断lookahead队列是否满了（zerolatency下也认为是满的）
void Lookahead::checkLookaheadQueue(int &frameCnt)
{
    ...
    if (!m_filled)
    {
        if (!m_param->bframes & !m_param->lookaheadDepth)  //不允许B帧，也没有lookahead机制
            m_filled = true; /* zero-latency */
        else if (frameCnt >= m_param->lookaheadDepth + 2 + m_param->bframes)
            m_filled = true; /* full capacity plus mini-gop lag */ //已经填满
    }
...
/* Called by API thread */
//返回当前帧类型决策完毕的待编码帧（没有可用帧则等待帧类型决策完毕再返回一帧数据）
Frame* Lookahead::getDecidedPicture()
{
    if (m_filled)  //当前lookahead队列已满
    {
        m_outputLock.acquire();
        Frame *out = m_outputQueue.popFront();   //抛出已经决策好帧类型的第一帧（用于编码）
        m_outputLock.release();
        ...
        m_inputLock.acquire();
        bool wait = m_outputSignalRequired = m_sliceTypeBusy;  //如果当前正在进行slicetype，则等待
        m_inputLock.release();

        if (wait)
            m_outputSignal.wait();  //一直等在帧类型决策完毕

        out = m_outputQueue.popFront();  //抛出已经决策好帧类型的第一帧（用于编码）
        if (out)
            m_inputCount--;
        return out;
    }
    else
        return NULL;  //lookahead队列未满，不能做帧类型决策，直接返回
}
...
//该函数只在待分析的帧没有低分辨率时被调用
//下采样生成低分辨率数据；AQ；lowresIntraEstimate
...
void Lookahead::slicetypeDecide()
{
    ...
    //frames[]对应下采样数据的信息，长度为lookachead其中frame[0]为m_lastNonB，Frame[1~depth] 为具体帧
    Lowres* frames[X265_LOOKAHEAD_MAX + X265_BFRAME_MAX + 4]; 
    //用于存储m_param->bframes + 2帧   用于寻找最优的b帧位置
    Frame*  list[X265_BFRAME_MAX + 4]; 
    memset(frames, 0, sizeof(frames));
    memset(list, 0, sizeof(list));
    int maxSearch = X265_MIN(m_param->lookaheadDepth, X265_LOOKAHEAD_MAX);  //最大前向分析的帧数
    maxSearch = X265_MAX(1, maxSearch);

    {
        ScopedLock lock(m_inputLock);

        //输入数据队列m_inputQueue放入寻找B帧的队列list中，长度是m_param->bframes + 2
        ...
        //获取输入数据队列m_inputQueue中的每一帧对应下采样数据的信息，长度为maxSearch
        ...
        for (j = 0; j < maxSearch; j++)
        {
            ...
            if (!curFrame->m_lowresInit)
                pre.m_preframes[pre.m_jobTotal++] = curFrame;  //PreLookaheadGroup类只处理没有将分辨率的帧
    ...
    {
        if (m_pool)
            pre.tryBondPeers(*m_pool, pre.m_jobTotal);  //唤醒线程
        pre.processTasks(-1);  //多线程轮流并行处理m_jobTotal个帧
        pre.waitForExit();
    }
...
//用下采样数据分析当前gop内的帧类型，并计算qpCuTreeOffset的值
void Lookahead::slicetypeAnalyse(Lowres **frames, bool bKeyframe)
{
    int numFrames, origNumFrames, keyintLimit, framecnt;
    int maxSearch = X265_MIN(m_param->lookaheadDepth, X265_LOOKAHEAD_MAX);  //最大需要分析的帧的个数
    int cuCount = m_8x8Blocks;  //以8x8为单元的块的个数
    int resetStart;
    bool bIsVbvLookahead = m_param->rc.vbvBufferSize && m_param->lookaheadDepth;  //是否需要vbv + lookahead

    /* count undecided frames */
    //统计列表中连续的帧类型没有被确定的帧的个数
    //下采样数据的[0]元素是最近的非B帧，所以从1开始统计
    for (framecnt = 0; framecnt < maxSearch; framecnt++)
    {
        Lowres *fenc = frames[framecnt + 1];
        if (!fenc || fenc->sliceType != X265_TYPE_AUTO)
            break;
    }  //索引号为1--framecnt为类型待确认帧序号

    if (!framecnt)
    {   //所有帧的类型都已经确定，则CuTree分析后退出
        if (m_param->rc.cuTree)
            cuTree(frames, 0, bKeyframe);
        return;
    }
    frames[framecnt + 1] = NULL;  //帧类型分析最大也只分析到framecnt为止
    ...
    int keylimit = m_param->keyframeMax;  //编码器参数的关键帧设置
    ...
    //frames[0]->frameNum表示显示顺序，frames[0]是I帧或者P帧
    //keyFrameLimit表示此时与下一个IDR帧之间的最大间隔帧数
    ...
    {   //说明再当前搜索区域第一帧就必须插入IDR帧
        frames[1]->sliceType = X265_TYPE_I;
        return;
    }

    //当满足条件：m_pool && m_param->bFrameAdaptive == X265_B_ADAPT_TRELLIS时会出发该操作
    if (m_bBatchMotionSearch)
    {
        /* pre-calculate all motion searches, using many worker threads */
        CostEstimateGroup estGroup(*this, frames);  //多线程计算
        ...
        m_bBatchMotionSearch &= m_pool->m_numWorkers >= 4;  //保证有4个及以上的内核才能进行多线程
    ...
    if (m_param->bHistBasedSceneCut)
        ...
        isScenecut = scenecut(frames, 0, 1, true, origNumFrames);  //判断是否存在场景切换
...
#define P_SENS_BIAS (50 - m_param->bFrameBias)
        ...
        {   //固定B帧模式
            ...
            //每隔numBFrames设置一个P帧，其余帧设置为B帧
            for (int j = 1; j < numFrames; j++)
                frames[j]->sliceType = (j % (numBFrames + 1)) ? X265_TYPE_B : X265_TYPE_P;

            frames[numFrames]->sliceType = X265_TYPE_P;  //当前搜索列表的最后一帧设置为P帧（因为双向参考的关系，最后一帧不可能是B帧）
        }
...
//判断当前是否场景切换（是否需要将p1帧类型设置为I帧），只在Lookahead::slicetypeAnalyse函数中被调用
//p0和p1为列表frames中相邻的前后两帧数据
bool Lookahead::scenecut(Lowres **frames, int p0, int p1, bool bRealScenecut, int numFrames)
{
    //需要真正的场景切换判断时，进行精细分析，否则跳过进行粗略分析
    if (bRealScenecut && m_param->bframes)
    {
        int origmaxp1 = p0 + 1;
        /* Look ahead to avoid coding short flashes as scenecuts. */
        origmaxp1 += m_param->bframes; //计数需要搜索P1的最大位置
...
bool Lookahead::scenecutInternal(Lowres **frames, int p0, int p1, bool bRealScenecut)
{
    ...
    int64_t icost = frame->costEst[0][0]; //获取后向帧的I帧编码cost（帧内编码）
    int64_t pcost = frame->costEst[p1 - p0][0]; ////获取当前帧与前向参考帧之间的最优frame cost（帧间编码）
    ...
    double bias = m_param->scenecutBias;  //值越大，越容易判断为场景切换，下面的if主要判断并计算bias
    if (bRealScenecut)
    {
...
void Lookahead::aqMotion(Lowres **frames, bool bIntra)
{
    if (!bIntra) //第一帧不是关键帧
    {
        ...
        while (frames[lastnonb]->sliceType != X265_TYPE_P)
            lastnonb++;
        bframes = lastnonb - 1;  //统计有多少个连续的B帧
...
void Lookahead::cuTree(Lowres **frames, int numframes, bool bIntra)
{
    ...
    //lastnonb记录GOP的后向非B帧位置；curnonb记录GOP的前向非B帧位置，所以0 <= curnonb <= 一堆B帧 <= lastnonb <= numframes
    ...
    for (int j = 0; j <= numframes; j++)
        totalDuration += (double)m_param->fpsDenom / m_param->fpsNum; //累加播放时长（cutree的计算有考虑时序播放上帧的重要性）
    ...
    lastnonb = i; //获取最后一个非B帧的位置（表示该帧是被其他帧所参考的）
    ...
    else
    {
        if (lastnonb < idx) //当前列表数据太少，直接退出
            return;
        //将最后一个非B帧全部的传播cost初始化为0，即下方注释中P1帧的传播cost
        memset(frames[lastnonb]->propagateCost, 0, m_cuCount * sizeof(uint16_t)); 
    }

    CostEstimateGroup estGroup(*this, frames);

    //从lookahead满足的序列的最后一帧开始遍历，往前计算传播cost
    while (i-- > idx) 
    {
        curnonb = i; //i,指向当前GOP最后一个B帧
        //获取下一个非B帧（实际上也是P帧）,表示该帧是被其他帧所参考的
        ...
        memset(frames[curnonb]->propagateCost, 0, m_cuCount * sizeof(uint16_t)); //将当前GOP的前向P帧的传播cost初始化为0
        bframes = lastnonb - curnonb - 1; //获取当前两P帧间的B帧个数
        if (m_param->bBPyramid && bframes > 1) //对应B帧可参考的情况，区别就是此处需要另外判断参考帧了！
        {
        ...
        else //对应B帧不可参考的情况，也即B帧只双向参考P帧，不参考其他B帧，因此参考帧都是固定的！
        {
            while (i > curnonb) 
            {
                ...
                i--; //指向下一帧，也即刚计算的B帧的前一帧（因为cutree的计算是从后往前计算的）
            }
        }//注意：后面在实际编码的过程中，参考关系会比这复杂得多，此处只是预分析
...
void CostEstimateGroup::estimateCUCost(LookaheadTLD& tld, int cuX, int cuY, int p0, int p1, int b, bool bDoSearch[2], bool lastRow, int slice, bool hme)
{
    ...
    // establish search bounds that don't cross extended frame boundaries 防止mv越界
