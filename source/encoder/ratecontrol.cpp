...
RateControl::RateControl(x265_param& p, Encoder *top)
{
    ...
    {
        m_param->rc.qp = (int)m_param->rc.rfConstant; //初始qp等于参数配置的crf值
        m_param->rc.bitrate = 0;

        double baseCplx = m_ncu * (m_param->bframes ? 120 : 80); //初始复杂度，与是否存在B帧有关
    ...
    m_bitrate = m_param->rc.bitrate * 1000; //获取当前配置的目标码率，默认值为0（也就是说选择ABR编码必须自己指定目标码率）
...
{
    /* Frame Predictors used in vbv */
    for (int i = 0; i < 4; i++) Slice predictors to preidct bits for each Slice type - I, P, Bref and B
    {
...
int RateControl::rateControlStart(Frame* curFrame, RateControlEntry* rce, Encoder* enc)
{
    ...
    m_curSlice = curEncData.m_slice; //获取当前帧的slice（all info about the current frame）
    m_sliceType = m_curSlice->m_sliceType; //获取当前的slice类型（即frame type）
    ...
    rce->poc = m_curSlice->m_poc; //当前poc
    ...
    bool isRefFrameScenecut = m_sliceType!= I_SLICE && m_curSlice->m_refFrameList[0][0]->m_lowres.bScenecut; //如果其前向[0]第一个[0]参考帧为场景切换帧
    ...
    rce->bufferRate = m_bufferRate; //平均每帧最大的bits数（配置的最大码率除以帧率）
    rce->rowCplxrSum = 0.0;
    rce->rowTotalBits = 0;
    ...
    if (m_isVbv)
    {
        ...
        m_predictedBits = m_totalBits; //已经编码帧的总bits数
...
{
    double q; //用于存储当前帧估计的qscale值，函数返回该值
    ...
    {
        if (m_isAbr) //只要不是CQP模式都会走该过程，功能：计算滑动窗口平均framecost和
        {
    ...
    if (m_isVbv && m_currentSatd > 0 && curFrame)
    {
            ...
            (m_param->bFrameAdaptive && m_param->bframes)) //应用lookachead 或者 应用cuTree 或者应用scencecut 或则 应用自适应B帧决策
        {
            ...
            * is in a reasonable state by the end of the lookahead. */
            ...
            //功能：此时还没真正编码该帧，根据下采样satd预测当前帧未来编码所需bits，从而修正qscale
            for (int iterations = 0; iterations < 1000 && loopTerminate != 3; iterations++)
            {
                double frameQ[3];
                double curBits;
                curBits = predictSize(&m_pred[m_predType], q, (double)m_currentSatd);  //预测当前帧的bits
                double bufferFillCur = m_bufferFill - curBits; //减去当前帧的预测编码bits后，剩余的bufferfill
                double targetFill;
                ...
                bool iter = true;
                for (int j = 0; bufferFillCur >= 0 && iter ; j++) //循环遍历未来帧占用的bits 直到遍历一秒钟 或者 buffer剩余为0
                {
        ...
        if (pbits > rce->frameSizeMaximum) //如果bits大于当前level定义值，调大qscale
...
* \参数 p                ： m_pred[predType]
* \参数 q                ： rce->qpaRc
* \参数 var              ： 当前帧的SATD值 framecost
* \参数 bits             ： 当前帧编码占用的实际bits
* \返回                  ： null * */
void RateControl::updatePredictor(Predictor *p, double q, double var, double bits)
{
    ...
    double new_coeff = X265_MAX((bits * q - old_offset) / var, p->coeffMin ); //用当前编码的实际bits估计原先占用的系数是多少
...
int RateControl::updateVbv(int64_t bits, RateControlEntry* rce)
{
    ...
    m_bufferFillFinal -= bits; //减去当前帧实际编码bits后，还剩余的预设量
