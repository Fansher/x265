...
* \参数 top        ： 上层Encoder类，上层Encoder类数据会传至下层FrameEncoder类
* \参数 numRows    ： 一帧的CTU行数
* \参数 numCols    ： 一帧的CTU列数
...
bool FrameEncoder::init(Encoder *top, int numRows, int numCols)
{
    ...
    m_param = top->m_param;
    m_numRows = numRows; //一帧数据种的CTU的行数和列数
    m_numCols = numCols;
    m_reconfigure = false;
...
void FrameEncoder::threadMain()
{
    ...
    if (m_pool)
    {
        ...
        if (!m_jpId)
        {//只在m_jpid = 0 时才会进入，也就说在framethread个线程中只有第一个才会进入 (因为这次初始化为把所有并行的analysis都初始化)
            int numTLD = m_pool->m_numWorkers; //获取当前机器核数
            if (!m_param->bEnableWavefront)
                numTLD += m_pool->m_numProviders;
...
void FrameEncoder::compressFrame()
{
    ...
    int numPredDir = slice->isInterP() ? 1 : slice->isInterB() ? 2 : 0;  //参考帧列表数，I帧不需要参考；P帧单向参考，只需一个list；B帧双向参考，list数为2
    ...
    int qp = m_top->m_rateControl->rateControlStart(m_frame, &m_rce, m_top);  //估计当前帧的量化参数
    m_rce.newQp = qp;
...
void FrameEncoder::processRow(int row, int threadId)
{
    ...
    const uint32_t typeNum = m_idx_to_row[row & 1]; //0 表示需要编码  1表示需要滤波
...
void FrameEncoder::processRowEncoder(int intRow, ThreadLocalData& tld)
{
    ...
    Slice *slice = curEncData.m_slice; //当前帧的slice

    const uint32_t numCols = m_numCols;  //当前行有多少CTU，即有多少列
    const uint32_t lineStartCUAddr = row * numCols; //获取当前CTU行的第一个CTU在帧中的号
    bool bIsVbv = m_param->rc.vbvBufferSize > 0 && m_param->rc.vbvMaxBitrate > 0; //vbv生效是maxrate和bufsize均设置

    const uint32_t sliceId = curRow.sliceId;
    uint32_t maxBlockCols = (m_frame->m_fencPic->m_picWidth + (16 - 1)) / 16;
    uint32_t noOfBlocks = m_param->maxCUSize / 16; //一个CTU块对应下采样8x8块的个数 的log2
