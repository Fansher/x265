...
#define MAX_FRAME_DURATION 1.00 //播放一帧需要的最长时间
#define MIN_FRAME_DURATION 0.01 //播放一帧需要的最短时间
...
struct Predictor
{
    //预测当前帧的bits (p->coeff * var + p->offset) / (q * p->count)   
    //q 一般为qscale  var 当前帧的framecost （可以是行的 也可以是8x8块的） 主要是 SATD与bits的映射关系
    double coeffMin;
...
struct RateControlEntry
{
    ...
    int64_t lastSatd;      /* Contains the picture cost of the previous frame, required for resetAbr and VBV */ //当前帧的SATD值 framecost
...
public:
    ...
    int         m_ncu;           /* number of CUs in a frame */  //1/2下采样帧中的8x8块个数
    int         m_qp;            /* updated qp for current frame */  //在rateControlStart中获取四舍五入的预估qp参数，经过码控后得到的m_qp，用于实际编码后的量化
    ...
    bool   m_isAbr;
    bool   m_isVbv; //m_param->rc.vbvMaxBitrate > 0 && m_param->rc.vbvBufferSize > 0
    bool   m_isCbr;
