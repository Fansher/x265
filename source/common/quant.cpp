                             ...
                             coeff_t* coeff, uint32_t log2TrSize, TextType ttype, uint32_t absPartIdx, bool useTransformSkip)
{
    ...
    //对变换/量化操作如果使用bypass操作，那么直接将残差数据拷贝到变换系数内存块中
    //coeff: 存储经变换/量化后的变换系数
    ...
    //如果跳过变换模式（useTransformSkip），则只需将残差数据进行相应移位
    if (useTransformSkip)
    {
#if X265_DEPTH <= 10
    ...
    else
    {
        ...
        //如果是帧内4x4亮度块，则进行dst变换；否则使用dct变换
        //变换后的系数存储在m_resiDctCoeff中
...
//ʹ��RDO�����Ա任���ϵ�������������ú���ֻ��Quant::transformNxN�����б����ã�����Ҫ��Ϊ3�����裺
//step1���Ե�ǰTU��ÿ��ϵ��������RDO�Ż����ҵ���ʧ�������ϵ���������ֵ
//step2���Ե�ǰTU��ÿһ��ϵ���飨CofficientGroup�������Ż�����һ������ͼ������CG��ϵ������Ϊ0���Դ�����ʧ�������ϵ�����
//step3���ҵ����ŵ����һ������ϵ����λ�ã�CABAC��Ҫ�������Դ����һ������ϵ����ʼ���������ϵ����Ϊ0
...
uint32_t Quant::rdoQuant(const CUData& cu, int16_t* dstCoeff, TextType ttype, uint32_t absPartIdx, bool usePsy)
{
    ...
    const int numCoeff = 1 << (log2TrSize * 2);   //��ǰTU��ϵ������
    uint32_t numSig = primitives.nquant(m_resiDctCoeff, qCoef, dstCoeff, qbits, add, numCoeff);            //�Ա任ϵ����������
    X265_CHECK((int)numSig == primitives.cu[log2TrSize - 2].count_nonzero(dstCoeff), "numSig differ\n");   //ͳ��������ķ���ϵ���������볣��������Ľ���Ƿ�һ��
    if (!numSig)   //�������ϵ��ȫΪ0����ֱ���˳�����rdoq
        return 0;

    const uint32_t trSize = 1 << log2TrSize;   //TU��С��Ҳ��ϵ������
...
#define PSYVALUE(rec)   ((psyScale * (rec)) >> X265_MAX(0, (2 * transformShift + 1)))
    ...
    //ϵ��������С��λ���Ŀ���
    int64_t costCoeff[trSize * trSize];   /* d*d + lambda * bits */  //ÿһ��ϵ���Ŀ���
    int64_t costUncoded[trSize * trSize]; /* d*d + lambda * 0    */  //ÿһ��ϵ��������Ϊ0�Ŀ�������Ӧʧ�沿�֣�
    int64_t costSig[trSize * trSize];     /* lambda * bits       */  //ÿһ��ϵ����ϵ����ǿ�������Ӧbits��
    ...
    uint64_t sigCoeffGroupFlag64 = 0;  //�������ÿһλ����CG�ķ����ǣ�ĳһλΪ1������Ӧ��CGϵ��ȫ0

    const uint32_t cgSize = (1 << MLS_CG_SIZE); /* 4x4 num coef = 16 */ //ϵ�����е�ϵ������
    bool bIsLuma = ttype == TEXT_LUMA;

    /* total rate distortion cost of transform block, as CBF=0 */
    int64_t totalUncodedCost = 0;  //��ǰ�鶼������Ϊ0ʱ�Ŀ���
    ...
    TUEntropyCodingParameters codeParams;
    cu.getTUEntropyCodingParameters(codeParams, absPartIdx, log2TrSize, bIsLuma);  //�õ��ر���Ĳ���
    ...
    uint8_t coeffNum[MLS_GRP_NUM];      // value range[0, 16]��ÿһ��CG�����ϵ������
    uint16_t coeffSign[MLS_GRP_NUM];    // bit mask map for non-zero coeff sign��ÿһ��CG��ķ���ϵ���ķ���
    uint16_t coeffFlag[MLS_GRP_NUM];    // bit mask map for non-zero coeff��ÿһ��CG��ķ���ϵ���ı��
    ...
    //��һ�����Ե�ǰTU��ÿ��ϵ��������RDO�Ż����ҵ���ʧ�������ϵ���������ֵ
    //�����һ������CG��ʼ����ÿ��CG�������һ������һ��
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0; cgScanPos--)
    {
        uint32_t ctxSet = (cgScanPos && bIsLuma) ? 2 : 0;
        const uint32_t cgBlkPos = codeParams.scanCG[cgScanPos];          //CG��λ��
        const uint32_t cgPosY   = cgBlkPos >> log2TrSizeCG;              //CG��ɨ��λ�õ�Y����
        const uint32_t cgPosX   = cgBlkPos & ((1 << log2TrSizeCG) - 1);  //CG��ɨ��λ�õ�X����
        const uint64_t cgBlkPosMask = ((uint64_t)1 << cgBlkPos);         // ��ǰCG��λ�ã�ʹ��cgBlkPosMask��1��λ������ʾ
        ...
        if (cgScanPos && (coeffNum[cgScanPos] == 0))
        {
            ...
            else
            {
                ...
                primitives.cu[log2TrSize - 2].nonPsyRdoQuant(m_resiDctCoeff, costUncoded, &totalUncodedCost, &totalRdCost, blkPos);
                blkPos = codeParams.scan[scanPosBase];
                for (int y = 0; y < MLS_CG_SIZE; y++)  //����CG���ÿ��ϵ��
                {
                    for (int x = 0; x < MLS_CG_SIZE; x++)
                    {
                        const uint32_t scanPosOffset =  y * MLS_CG_SIZE + x;
                        ...
                        costSig[scanPosBase + scanPosOffset] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);
                        costCoeff[scanPosBase + scanPosOffset] = costUncoded[blkPos + x];  //����ȫ���CG��ÿ��ϵ��������������Ϊ0��ʧ�沿��
                        sigRateDelta[blkPos + x] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig]; //ϵ��������Ϊ0�ͷ�0��ϵ����ǵ�bit����
                    }
                    blkPos += trSize;  
                }
            }
        ...
        {
            scanPos              = (cgScanPos << MLS_CG_SIZE) + scanPosinCG;  //ϵ����Z��ɨ��˳��
            uint32_t blkPos      = codeParams.scan[scanPos];                  //��Ӧ��ɨ��λ��
            uint32_t maxAbsLevel = dstCoeff[blkPos];                  /* abs(quantized coeff) */           //��������ֵ
            int signCoef         = m_resiDctCoeff[blkPos];            /* pre-quantization DCT coeff */     //DCTϵ��
            ...
            costUncoded[blkPos] = ((int64_t)signCoef * signCoef) << scaleBits;  //�õ�ϵ������Ϊ0�Ŀ���
            ...
            totalUncodedCost += costUncoded[blkPos];  //�ۼ�ϵ������Ϊ0�Ŀ���
            ...
            if (scanPos > (uint32_t)lastScanPos)  //�����ǰλ�������һ������ϵ��֮��
            {
                /* coefficients after lastNZ have no distortion signal cost */
                costCoeff[scanPos] = 0;  
                costSig[scanPos] = 0;
            ...
            else if (!(subFlagMask & 1))  //�����ǰϵ�������һ������ϵ��֮ǰ�����Ҹ�ϵ��Ϊ0�����㿪��
            {
                // fast zero coeff path
                /* set default costs to uncoded costs */
                costSig[scanPos] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);  //���ϵ���Ŀ���
                costCoeff[scanPos] = costUncoded[blkPos] + costSig[scanPos];         //�ܵĿ���
                sigRateDelta[blkPos] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig];
                totalRdCost += costCoeff[scanPos];  //�ۼ�ϵ�����ܿ���
                rateIncUp[blkPos] = greaterOneBits[0];

                subFlagMask >>= 1;  //ϵ�����λ���ƣ�ȡ��ǰCG����һ��ϵ���ķ�����
            }
            else  //�����ǰϵ�������һ������ϵ��֮ǰ������ϵ����Ϊ0������ÿһ��ϵ������������ֵ
            {
                subFlagMask >>= 1;
                ...
                uint32_t level = 0;               //��ǰϵ����ֵ
                uint32_t sigCoefBits = 0;         //ϵ����ǵĿ���
                costCoeff[scanPos] = MAX_INT64;   //��ǰϵ�����ܿ�����ʧ���bit����ͳ�ƣ�

                if ((int)scanPos == lastScanPos)
                    sigRateDelta[blkPos] = 0;
                else
                {
                    if (maxAbsLevel < 3)  //�����������ϵ��ֵ�Ƚ�С���ͳ�������Ϊ0���бȽ�
                    { 
                        /* set default costs to uncoded costs */
                        costSig[scanPos] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);  //����Ϊ0ʱϵ����ǵĿ���
                        costCoeff[scanPos] = costUncoded[blkPos] + costSig[scanPos];         //�ܿ��� = ����Ϊ0��ʧ�� + ����Ϊ0�Ŀ�������ʱ��û�з�ֵ���ֵĿ�����
                    }
                    sigRateDelta[blkPos] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig];
                    sigCoefBits = estBitsSbac.significantBits[1][ctxSig];   //û������ʱ��*bits���ֵĿ���
                }
                ...
                if (maxAbsLevel == 1)   //���������ϵ��Ϊ1ʱ����ϵ������Ϊ0ʱ������ʧ��Ƚ�
                {
                    uint32_t levelBits = (c1c2idx & 1) ? greaterOneBits[0] + IEP_RATE : ((1 + goRiceParam) << 15) + IEP_RATE;  //����Ϊ1ʱ��ϵ����ֵ��bit����
                    ...
                    int unquantAbsLevel = unQuantLevel >> unquantShift;                 //�õ�ϵ��Ϊ1ʱ�ķ�����ֵ
                    X265_CHECK(UNQUANT(1) == unquantAbsLevel, "DQuant check failed\n"); 
                    int d = abs(signCoef) - unquantAbsLevel;                            //�õ�ʧ��
                    int64_t curCost = RDCOST(d, sigCoefBits + levelBits);               //����ϵ������Ϊ1�Ŀ�������ʱbit������ϵ����ǿ�����ϵ����ֵ������
                    ...
                    {
                        level = 1;                                 //���µ�ǰ����ֵ
                        costCoeff[scanPos] = curCost;              //�����ܵĿ���
                        costSig[scanPos] = SIGCOST(sigCoefBits);   //ϵ����ǿ���
                    }
                }
                else if (maxAbsLevel)
                {   //���������ϵ������1ʱ�����㵱ǰϵ����ϵ��-1���2�������bits
                ...
                dstCoeff[blkPos] = (int16_t)level;  //��ʱ�Ե�ǰϵ��������RDO�Ż�������Ŀ������ֵ
    ...
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0 && !foundLast; cgScanPos--)
    {
        ...
        for (int scanPosinCG = cgSize - 1; scanPosinCG >= 0; scanPosinCG--)
        {
            scanPos = cgScanPos * cgSize + scanPosinCG;  //ϵ����ɨ��λ��
            if ((int)scanPos > lastScanPos)  //lastScanPos�ǡ����һ������ϵ������ɨ��λ��
                continue;
            ...
            if (dstCoeff[blkPos])  //Ŀ������ϵ������0������ͼ����ϵ������Ϊ0
            {
                // Calculates the cost of signaling the last significant coefficient in the block 
                uint32_t pos[2] = { (blkPos & (trSize - 1)), (blkPos >> log2TrSize) };  //��ϵ����X/Yλ��
                ...
                {  //�����λ�õ�X/Y��������Ҫ��bit���ѣ�
                    ...
                    int prefixOnes = temp & 15;  //ǰ׺
                    int suffixLen = temp >> 4;   //��׺

                    bitsLastNZ += m_entropyCoder->m_estBitsSbac.lastBits[i][prefixOnes];  //ǰ׺�ر������ĵ�bit
                    bitsLastNZ += IEP_RATE * suffixLen;  //��׺�ر������ĵ�bit
                }
