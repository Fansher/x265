...
//使用RDO技术对变换后的系数进行量化（该函数只在Quant::transformNxN函数中被调用），主要分为3个步骤：
//step1：对当前TU的每个系数单独做RDO优化，找到率失真意义上的最优量化值
//step2：对当前TU的每一个系数组（CofficientGroup）进行优化，这一步是试图将整个CG的系数都置为0，以带来率失真意义上的增益
//step3：找到最优的最后一个非零系数的位置（CABAC需要），尝试从最后一个非零系数开始将量化后的系数置为0
...
uint32_t Quant::rdoQuant(const CUData& cu, int16_t* dstCoeff, TextType ttype, uint32_t absPartIdx, bool usePsy)
{
    ...
    const int numCoeff = 1 << (log2TrSize * 2);   //当前TU的系数个数
    uint32_t numSig = primitives.nquant(m_resiDctCoeff, qCoef, dstCoeff, qbits, add, numCoeff);            //对变换系数常规量化
    X265_CHECK((int)numSig == primitives.cu[log2TrSize - 2].count_nonzero(dstCoeff), "numSig differ\n");   //统计量化后的非零系数个数，与常规量化后的结果是否一致
    if (!numSig)   //量化后的系数全为0，则直接退出不做rdoq
        return 0;

    const uint32_t trSize = 1 << log2TrSize;   //TU大小，也即系数个数
...
#define PSYVALUE(rec)   ((psyScale * (rec)) >> X265_MAX(0, (2 * transformShift + 1)))
    ...
    //系数级别（最小单位）的开销
    int64_t costCoeff[trSize * trSize];   /* d*d + lambda * bits */  //每一个系数的开销
    int64_t costUncoded[trSize * trSize]; /* d*d + lambda * 0    */  //每一个系数被量化为0的开销（对应失真部分）
    int64_t costSig[trSize * trSize];     /* lambda * bits       */  //每一个系数的系数标记开销（对应bits）
    ...
    uint64_t sigCoeffGroupFlag64 = 0;  //好巧妙！用每一位代表CG的非零标记，某一位为1代表对应的CG系数全0

    const uint32_t cgSize = (1 << MLS_CG_SIZE); /* 4x4 num coef = 16 */ //系数组中的系数个数
    bool bIsLuma = ttype == TEXT_LUMA;

    /* total rate distortion cost of transform block, as CBF=0 */
    int64_t totalUncodedCost = 0;  //当前块都被量化为0时的开销
    ...
    TUEntropyCodingParameters codeParams;
    cu.getTUEntropyCodingParameters(codeParams, absPartIdx, log2TrSize, bIsLuma);  //得到熵编码的参数
    ...
    uint8_t coeffNum[MLS_GRP_NUM];      // value range[0, 16]，每一个CG组非零系数个数
    uint16_t coeffSign[MLS_GRP_NUM];    // bit mask map for non-zero coeff sign，每一个CG组的非零系数的符号
    uint16_t coeffFlag[MLS_GRP_NUM];    // bit mask map for non-zero coeff，每一个CG组的非零系数的标记
    ...
    //第一步：对当前TU的每个系数单独做RDO优化，找到率失真意义上的最优量化值
    //从最后一个非零CG开始遍历每个CG，从最后一个到第一个
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0; cgScanPos--)
    {
        uint32_t ctxSet = (cgScanPos && bIsLuma) ? 2 : 0;
        const uint32_t cgBlkPos = codeParams.scanCG[cgScanPos];          //CG的位置
        const uint32_t cgPosY   = cgBlkPos >> log2TrSizeCG;              //CG的扫描位置的Y坐标
        const uint32_t cgPosX   = cgBlkPos & ((1 << log2TrSizeCG) - 1);  //CG的扫描位置的X坐标
        ...
        if (cgScanPos && (coeffNum[cgScanPos] == 0))
        {
            ...
            else
            {
                ...
                primitives.cu[log2TrSize - 2].nonPsyRdoQuant(m_resiDctCoeff, costUncoded, &totalUncodedCost, &totalRdCost, blkPos);
                blkPos = codeParams.scan[scanPosBase];
                for (int y = 0; y < MLS_CG_SIZE; y++)  //遍历CG组的每个系数
                {
                    for (int x = 0; x < MLS_CG_SIZE; x++)
                    {
                        const uint32_t scanPosOffset =  y * MLS_CG_SIZE + x;
                        ...
                        costSig[scanPosBase + scanPosOffset] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);
                        costCoeff[scanPosBase + scanPosOffset] = costUncoded[blkPos + x];  //对于全零的CG，每个系数的消耗是量化为0的失真部分
                        sigRateDelta[blkPos + x] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig]; //系数被量化为0和非0，系数标记的bit消耗
                    }
                    blkPos += trSize;  
                }
            }
            ...
            continue;  //本身就是全零系数组的话，下面的部分就完全不需要了
        }

        coeffGroupRDStats cgRdStats;
        memset(&cgRdStats, 0, sizeof(coeffGroupRDStats));

        uint32_t subFlagMask = coeffFlag[cgScanPos];  //一个CG内系数的标记
        int    c2            = 0;
        ...
        {
            scanPos              = (cgScanPos << MLS_CG_SIZE) + scanPosinCG;  //系数的Z型扫描顺序
            uint32_t blkPos      = codeParams.scan[scanPos];                  //对应的扫描位置
            uint32_t maxAbsLevel = dstCoeff[blkPos];                  /* abs(quantized coeff) */           //常规量化值
            int signCoef         = m_resiDctCoeff[blkPos];            /* pre-quantization DCT coeff */     //DCT系数
            ...
            costUncoded[blkPos] = ((int64_t)signCoef * signCoef) << scaleBits;  //得到系数量化为0的开销
            ...
            totalUncodedCost += costUncoded[blkPos];  //累加系数量化为0的开销
            ...
            if (scanPos > (uint32_t)lastScanPos)  //如果当前位置在最后一个非零系数之后
            {
                /* coefficients after lastNZ have no distortion signal cost */
                costCoeff[scanPos] = 0;  
                costSig[scanPos] = 0;
            ...
            else if (!(subFlagMask & 1))  //如果当前系数在最后一个非零系数之前，并且该系数为0，计算开销
            {
                // fast zero coeff path
                /* set default costs to uncoded costs */
                costSig[scanPos] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);  //标记系数的开销
                costCoeff[scanPos] = costUncoded[blkPos] + costSig[scanPos];         //总的开销
                sigRateDelta[blkPos] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig];
                totalRdCost += costCoeff[scanPos];  //累计系数的总开销
                rateIncUp[blkPos] = greaterOneBits[0];

                subFlagMask >>= 1;  //系数标记位右移，取当前CG内下一个系数的非零标记
            }
            else  //如果当前系数在最后一个非零系数之前，并且系数不为0，估计每一个系数的最优量化值
            {
                subFlagMask >>= 1;
                ...
                uint32_t level = 0;               //当前系数幅值
                uint32_t sigCoefBits = 0;         //系数标记的开销
                costCoeff[scanPos] = MAX_INT64;   //当前系数的总开销（失真和bit均有统计）

                if ((int)scanPos == lastScanPos)
                    sigRateDelta[blkPos] = 0;
                else
                {
                    if (maxAbsLevel < 3)  //如果常规量化系数值比较小，就尝试量化为0进行比较
                    { 
                        /* set default costs to uncoded costs */
                        costSig[scanPos] = SIGCOST(estBitsSbac.significantBits[0][ctxSig]);  //量化为0时系数标记的开销
                        costCoeff[scanPos] = costUncoded[blkPos] + costSig[scanPos];         //总开销 = 量化为0的失真 + 量化为0的开销，此时就没有幅值部分的开销了
                    }
                    sigRateDelta[blkPos] = estBitsSbac.significantBits[1][ctxSig] - estBitsSbac.significantBits[0][ctxSig];
                    sigCoefBits = estBitsSbac.significantBits[1][ctxSig];   //没有量化时λ*bits部分的开销
                }
                ...
                if (maxAbsLevel == 1)   //当量化后的系数为1时，与系数量化为0时进行率失真比较
                {
                    uint32_t levelBits = (c1c2idx & 1) ? greaterOneBits[0] + IEP_RATE : ((1 + goRiceParam) << 15) + IEP_RATE;  //量化为1时，系数幅值的bit开销
                    ...
                    int unquantAbsLevel = unQuantLevel >> unquantShift;                 //得到系数为1时的反量化值
                    X265_CHECK(UNQUANT(1) == unquantAbsLevel, "DQuant check failed\n"); 
                    int d = abs(signCoef) - unquantAbsLevel;                            //得到失真
                    int64_t curCost = RDCOST(d, sigCoefBits + levelBits);               //计算系数量化为1的开销（此时bit部分有系数标记开销和系数幅值开销）
                    ...
                    {
                        level = 1;                                 //更新当前量化值
                        costCoeff[scanPos] = curCost;              //更新总的开销
                        costSig[scanPos] = SIGCOST(sigCoefBits);   //系数标记开销
                    }
                }
                else if (maxAbsLevel)
                {   //当量化后的系数大于1时，计算当前系数和系数-1后的2种情况的bits
                ...
                dstCoeff[blkPos] = (int16_t)level;  //此时对当前系数已昨晚RDO优化，更新目标量化值
    ...
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0 && !foundLast; cgScanPos--)
    {
        ...
        //内层循环：遍历CG内的系数，从最后一个系数开始
        for (int scanPosinCG = cgSize - 1; scanPosinCG >= 0; scanPosinCG--)
        {
            scanPos = cgScanPos * cgSize + scanPosinCG;  //系数的扫描位置
            if ((int)scanPos > lastScanPos)  //lastScanPos是“最后一个非零系数”的扫描位置
                continue;
            ...
            if (dstCoeff[blkPos])  //目标量化系数不是0，则试图将该系数设置为0
            {
                // Calculates the cost of signaling the last significant coefficient in the block 
                uint32_t pos[2] = { (blkPos & (trSize - 1)), (blkPos >> log2TrSize) };  //该系数的X/Y位置
                if (codeParams.scanType == SCAN_VER)
                    std::swap(pos[0], pos[1]);
                uint32_t bitsLastNZ = 0;

                for (int i = 0; i < 2; i++)
                {  //估算该位置的X/Y坐标所需要的bit花费，
                    int temp = g_lastCoeffTable[pos[i]];
                    int prefixOnes = temp & 15;  //前缀
                    int suffixLen = temp >> 4;   //后缀

                    bitsLastNZ += m_entropyCoder->m_estBitsSbac.lastBits[i][prefixOnes];  //前缀熵编码消耗的bit
                    bitsLastNZ += IEP_RATE * suffixLen;  //后缀熵编码消耗的bit
                }
